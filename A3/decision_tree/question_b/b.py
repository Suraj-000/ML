...
def plot(max_depths,train_acc,test_acc,val_acc):
    ...
    for i, depth in enumerate(max_depths):
        ...
        plt.text(depth, val_acc[i] - 0.01, f"{val_acc[i]:.3f}", ha='center',va='bottom', color='green', fontsize=9,bbox=dict(facecolor='white', edgecolor='none', alpha=0.7, pad=1))
        plt.text(depth, test_acc[i] + 0.01, f"{test_acc[i]:.3f}", ha='center', va='top', color='orange', fontsize=9,bbox=dict(facecolor='white', edgecolor='none', alpha=0.7, pad=1))
    plt.xlabel('Maximum Depth of Tree')
    plt.ylabel('Accuracy')
    plt.title('Decision Tree Accuracy vs Maximum Depth')
    plt.legend()
    plt.grid(True)
    plot_path=os.path.join("../plots","Q2_accuracy_vs_depth.png")
...
def main():
    ...
    print(f"Q2. Decision tree construction with One hot encoding")
    ...
    for d in max_depths:
        ...
        testPredictions.append(test_pred)        
    ...
    plot(max_depths,train_acc,test_acc,val_acc)

    idxx=np.argmax(val_acc)
    best_depth=max_depths[idxx]
    save_result_in_csv(output_path,testPredictions[idxx])
    print(f'best depth is {best_depth} and is saved in output folder')
    end_time=time.time()
