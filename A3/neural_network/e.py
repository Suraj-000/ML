...
def plot(f1_scores_train,f1_scores_test,hidden_sizes):
    ...
    plot_path=os.path.join("plots","F1_score_vs_depth_relu_ePart.png")
...
def main():
    ...
    csv_path=os.path.join(output_path,'prediction_e.csv')
    ...
    for i,hidden_layers in enumerate(all_depths,1):
        print_star()
        a=time.time()
        print(f'Training for hidden layers = {hidden_layers}')
        ...
        print_star()
        b=time.time()
        print(f'Total execution time for hidden units {hidden_layers} is {((b-a)/60.0):.4f} minutes')
    print()
